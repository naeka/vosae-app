...
class QuotationResourceTest(InvoiceBaseResourceTest):
    ...
    def test_08_make_down_payment_invoice(self):
        ...
        self.assertValidJSONResponse(response)
        deserialized = self.deserialize(response)
        self.assertEqual(len(deserialized['down_payments']), 1)
    ...
    def test_09_make_invoice(self):
        ...
        response = self.api_client.get(deserialized.get('related_quotation'), format='json')
        self.assertValidJSONResponse(response)
        deserialized_quotation = self.deserialize(response)
        self.assertEqual(deserialized_quotation['state'], u'INVOICED')
        self.assertEqual(deserialized_quotation['related_invoice'], deserialized.get('resource_uri'))
        self.assertEqual(len(deserialized_quotation['down_payments']), 1)
        ...
        self.assertEqual(deserialized_quotation['related_invoice'], None)
        self.assertEqual(len(deserialized_quotation['down_payments']), 1)
...
class DownPaymentInvoiceResourceTest(InvoiceBaseResourceTest):
    ...
    def setUpClass(cls):
        ...
        quotation.save()
        down_payment_invoice_1 = quotation.make_down_payment(
            issuer=settings.VOSAE_USER,
            percentage=decimal.Decimal('0.3'),
            tax=item.tax,
            date=datetime.date.today()
        )
        down_payment_invoice_2 = quotation.make_down_payment(
            issuer=settings.VOSAE_USER,
            percentage=decimal.Decimal('0.3'),
            tax=item.tax,
            date=datetime.date.today()
        )
        down_payment_invoice_3 = quotation.make_down_payment(
            issuer=settings.VOSAE_USER,
            percentage=decimal.Decimal('0.3'),
            tax=item.tax,
            date=datetime.date.today()
