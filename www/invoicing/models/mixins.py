...
class InvoiceMakableMixin(object):
    ...
    def make_down_payment_invoice(self, issuer, percentage, tax, date):
        ...
        # Initialize the down-payment invoice
        ...
        # Fill the translatable invoice item
        invoice_item = InvoiceItem(
            reference=down_payment_invoice.ITEM_REFERENCE,
            quantity=1,
            unit_price=(down_payment_amount / (Decimal('1.00') + tax.rate)).quantize(Decimal('1.00'), ROUND_HALF_UP),
            tax=tax,
            is_translatable=True
        )
        invoice_item.description, invoice_item.translation_context = down_payment_invoice.get_item_description()
